...
const float targetDist = 817.0;           // Target distance in centimeters
const float targetTime = 15.3;            // Target time in seconds to reach distance
// lowkey i think the time doesnt work that well, but maybe at penn our battery was just low

//servo isn't used anymore
...
const float gearRatio = 6.0;                                                  // Gear ratio (encoder to wheel)
...
void setup() {
  ...
  double velKp = 0.003, velKi = 0.00, velKd = 0.0;
  velocityPID.begin(&velInput, &velOutput, &velSetpoint, velKp, velKi, velKd);
  velocityPID.setOutputLimits(0.0001, 1.0);
...
void loop() {
  ...
  if (driving) {
    ...
    // Note: Encoder counts backwards, so subtract to accumulate positive distance
    ...
    // Note: Negate distanceTraveled because encoder counts backwards
    if (deltaTime > 0) {
